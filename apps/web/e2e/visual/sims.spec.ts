...
// #409: la story `Topic`, el widget con las props que escribe un MDX de tema, pausada en `t = 0`.
const LINE_FOLLOWER_SECTION = 'LineFollowerWidget';
const LINE_FOLLOWER_SHOTS = [
  { story: 'Oval', shot: 'LineFollowerWidget' },
  { story: 'Topic', shot: 'LineFollowerWidget-topic' },
] as const;

for (const { story, shot } of LINE_FOLLOWER_SHOTS)
  test(`${shot} looks as approved`, async ({ page }) => {
    await page.emulateMedia({ colorScheme: 'light' });
    ...
    const target = page.locator(
      `[data-section="${LINE_FOLLOWER_SECTION}"] [data-story="${story}"]`,
    );
    await expect(target).toBeVisible();
