...
describe('csvEscape (F3-02b)', () => {
  ...
  it('F3-02b golden: guards a field starting with a formula prefix (OWASP CSV injection)', () => {
    expect(csvEscape('=SUM(A1)')).toBe('"\'=SUM(A1)"');
    expect(csvEscape('-1')).toBe('"\'-1"');
    expect(csvEscape('@x')).toBe('"\'@x"');
  });

  it('F3-02b golden: numbers generated by the code itself are not guarded', () => {
    expect(csvEscape('1.00')).toBe('1.00');
    expect(csvEscape('1')).toBe('1');
  });
});
