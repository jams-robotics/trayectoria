...
Distribución de los simuladores de Explora (#351): aplica a todo widget con visor y panel. Columna izquierda: visor, `SimControls` y, debajo, el panel o los paneles de parámetros (`ParamPanel` y los selectores propios del widget). Columna derecha (`w-panel`): solo el panel de valores y su descripción en vivo. Con dos juegos de parámetros (A/B), sus paneles van lado a lado bajo el visor cuando cada uno dispone de al menos 280 px de ancho (rejilla `repeat(auto-fit, minmax(280px, 1fr))`); si no, uno debajo del otro. Objetivo: al mover un control se ve el visor sin desplazarse. En móvil no cambia nada (§9: una columna). Los widgets sin visor (p. ej. `EnergyWidget` en `power`) quedan fuera de esta regla.
