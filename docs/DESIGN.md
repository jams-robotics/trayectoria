...
Distribución de los simuladores de Explora (#369, sustituye a la de #351): aplica a todo widget con visor y panel, desde el punto de corte en que el widget parte en dos columnas (`lg` salvo que el widget use `md`). Los widgets sin visor (p. ej. `EnergyWidget` en `power`) quedan fuera de esta regla.
1. **Fila superior.** Izquierda: visor y `SimControls`. Derecha (`w-panel`): solo el panel de valores y su descripción en vivo. El alto de la fila es el mayor entre la columna del visor y el panel de valores, con el panel limitado a la altura máxima del visor (50 vh): la fila puede crecer con el panel hasta ese tope, y solo si el panel pasa de él se desplaza por dentro (`overflow-y: auto`) (#386).
2. **Controles.** Debajo de la fila superior y a todo el ancho del widget: los paneles de parámetros (`ParamPanel` y los selectores propios del widget) en una rejilla de 2 columnas (`grid-cols-2`, gap 16). Con dos juegos (A/B), A a la izquierda y B a la derecha. Un panel único con 4 o más sliders ocupa las 2 columnas y reparte sus sliders en 2 columnas, en orden de lectura por filas; con menos de 4 ocupa una columna.
3. **Contenido extra.** Gráficas y paneles secundarios (p. ej. el «panel de curva») van debajo de los controles, a todo el ancho.
4. **Visor fijo.** La fila superior completa (visor, `SimControls` y panel de valores) es `position: sticky; top: 0` con fondo `bg`, `z-index` sobre los controles y 12 px de relleno inferior; los controles y el contenido extra pasan por debajo al desplazarse y la fila se suelta al llegar al final del widget. Altura máxima del visor: 50 vh (su ancho se limita a `50vh · 16/9` y se centra en su columna). Los controles enfocables de los paneles llevan `scroll-margin-top: calc(50vh + 64px)` para que el foco de teclado no quede tapado (§8). El sticky se desactiva bajo el punto de corte y con ventanas de menos de 640 px de alto (`(max-height: 639px)`), donde la fila fija dejaría sin sitio a los controles.
5. **Móvil.** Una columna, sin sticky; el orden no cambia (§9).

Páginas de simulador (`/simuladores/movil` y `/simuladores/brazo`, #369): desde el punto de corte en que la página parte en dos columnas (`lg` en la del móvil, `md` en la del brazo), columna izquierda con el visor y, debajo, las gráficas del seguidor de línea (panel «Gráficas») o el panel de matrices del brazo, para no dejar hueco vacío bajo el visor; columna derecha (`w-panel`) con el resto de controles y paneles. La columna izquierda completa es `position: sticky; top: 0` mientras se desplaza la derecha, con la misma altura máxima del visor (50 vh) y el mismo corte de 640 px de alto que los widgets; si la columna izquierda es más alta que la ventana, se desplaza por dentro (`max-height: 100vh; overflow-y: auto`). En el editor de pista la regla no cambia: el editor ocupa la caja del visor. En móvil no cambia nada (§9): una columna, sin sticky, y las gráficas y las matrices siguen en su sitio actual.
