...
1. **Fila superior.** Izquierda: visor y `SimControls`. Derecha (`w-panel`): solo el panel de valores y su descripción en vivo. El alto de la fila lo marca la columna del visor; si el panel de valores es más alto, se desplaza por dentro (`overflow-y: auto`) en lugar de alargar la fila.
