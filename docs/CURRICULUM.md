...
Resuelto en #396 (DOCS-M6). Dos tickets de código van antes de T-6.2: el que expone `LineFollowerWidget` en el mapa MDX (`WIDGETS.md`) y el de la respuesta del motor (`ARCHITECTURE.md` §4.1). Los resultados que dependen de la respuesta del motor llevan la marca *a verificar en el simulador tras el ticket de código*. El ticket del tema los mide en el simulador y los escribe tal como salen; si uno contradice lo que dice la spec, es un spec gap.
...
- Explora: `LineFollowerWidget track="oval" controller="p" initialParams={{ kp: 8, omegaBase_radps: 15 }} showPlots={['error','omega']}` (sin `compact`, #396). Experimentos: (1) pon Kp = 2 → observa la primera curva → el robot se sale. (2) pon Kp = 20 → observa el error en las rectas → oscila de lado a lado: el motor tarda en cambiar de velocidad y la corrección llega tarde (a verificar en el simulador tras el ticket de código). (3) cambia a on/off → observa la traza y el error → zigzag permanente aunque la recta sea perfecta (a verificar en el simulador tras el ticket de código). Al cambiar de controlador, el widget vuelve a los valores por defecto del nuevo (`ω_base = 10`, `u_0 = 4`); por eso on/off va al final.
...
- Explora: `LineFollowerWidget track="tight" controller="pid" initialParams={{ kp: 20, ki: 0, kd: 0, omegaBase_radps: 13 }} showPlots={['error','pid']}` (sin `compact`, #396). Experimentos: (1) sube Kd de 0 a 0.8 → observa el error en las rectas → la oscilación se amortigua (quedan los cambios de signo de las esquinas). (2) con Kd = 0.8 y Ki = 0, observa el término I y el error en las curvas → el error tiene un sesgo constante hacia afuera; pon Ki = 2 y luego 10 → el sesgo se reduce. (3) con Ki = 10, baja «Límite integral» de 1 a 0.02 → vuelve el sesgo: el anti-windup limita cuánto corrige I. Los tres resultados, a verificar en el simulador tras el ticket de código.
...
- Explora: `LineFollowerWidget track="tight" controller="pid" initialParams={{ kp: 8, ki: 1, kd: 0.05, omegaBase_radps: 13 }} showPlots={['error']}` (sin `compact`, #396) con `MyRobotWidget mode="form"` al lado (el widget lee el perfil en vivo). Antes de empezar, el texto pide anotar los valores del perfil; al final, restaurarlos, porque «Guardar» sobrescribe el perfil real. Experimentos: (1) sube `forwardOffset_m` de 0.09 a 0.15 → observa el error y el tiempo de vuelta → el mismo desvío de rumbo da un p mayor (p ∝ d) y el robot corrige antes. (2) baja `count` a 3 → observa el error en las curvas → sube: la línea se acerca al borde del arreglo. (3) sube `wheelBase_m` a 0.25 → observa la primera curva → el robot se sale: con el mismo u, `ω = 2ur/L` es menor. Las cifras de los tres (tiempo de vuelta, error en curva), a verificar en el simulador tras el ticket de código.
...
- Explora: `LineFollowerWidget track="oval" controller="pid" initialParams={{ kp: 8, ki: 1, kd: 0.05, omegaBase_radps: 15 }} showPlots={['error','v','pid']}`. La longitud de `oval` (2.771 m) la da el texto, porque el widget no la muestra. Con esos parámetros, el robot de referencia debe completar la vuelta, con ruido y sin él (a verificar en el simulador tras el ticket de código; es también criterio de ese ticket). Experimentos: los 5 pasos del guion. En el paso 5, el texto guía la comparación del `t_vuelta` y la «Velocidad media» que mide el estudiante con su predicción: calcula `v_med` y `Δ%` con sus números, sin ejercicio (#396).
- Al robot: la predicción con el perfil del estudiante. Referencia: `ω_base = 15` → `v_pred = 0.48 m/s`; `oval` de 2.771 m → `t_pred = 5.773 s` (#396). El tiempo de vuelta del robot de referencia con los parámetros de Explora, a verificar en el simulador tras el ticket de código.
